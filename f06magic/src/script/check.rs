...
use crate::script::predicate::{EvalOutcome, Predicate, Stats};
...
pub(crate) struct Check {
  ...
  /// User-supplied boolean predicate; PASS iff the predicate evaluates to
  /// a non-zero, non-NaN value. See `script::predicate` for the grammar.
...
pub(crate) enum CheckRule {
  ...
  /// `predicate` rule: the user-supplied predicate returned a fail value
...
impl Check {
  ...
  /// `predicate` against the precomputed `stats` (both must be `Some` or
    ...
    I: IntoIterator<Item = (DatumIndex, F06Number)>,
  {
    ...
    for (i, (di, x_tmp)) in numbers.into_iter().enumerate() {
      ...
      if let (Some(eq), Some(st)) = (predicate, stats) {
